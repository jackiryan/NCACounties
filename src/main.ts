...
const bordersStyleFunction = (feature: FeatureLike): Style | undefined => {
    const layer = feature.get('layer');
...
const bordersLayer = new VectorTileLayer({
    source: new VectorTileSource({
        format: new MVT(),
        url: `https://api.maptiler.com/tiles/v3-lite/{z}/{x}/{y}.pbf?key=${key}`,
        maxZoom: 18,
    }),
    style: bordersStyleFunction,
    declutter: true,
    renderBuffer: 128,
...
const map = new OLMap({
    target: 'map',
    layers: [baseLayer, bordersLayer, countiesLayer, labelsLayer],
    view: new View({
